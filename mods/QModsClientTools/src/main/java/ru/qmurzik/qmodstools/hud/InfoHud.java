...
public final class InfoHud {
    ...
    public InfoHud(Minecraft mc, CpsCounter cps, PingMeter ping) { this.mc = mc; this.cps = cps; this.ping = ping; }

    public void render() {
        if (mc.thePlayer == null || mc.currentScreen != null) return;
        ScaledResolution sr = new ScaledResolution(mc); int y = 6;
        if (QModsTools.config.pingHud) {
            int p = ping.getPing(); int color = p < 0 ? 0xAAAAAA : p < 80 ? 0x55FF88 : p < 180 ? 0xFFD24D : 0xFF5577;
            y = badge(sr, p < 0 ? "§7... мс" : String.format("§f%d §7мс", p), y, color);
        }
